...
| Cache / drain layer | `Services/CommandTracker.cs` (`_cachedResults`, `FlipToCacheMode`, `ConsumeCachedOutputs`, `BuildStatusLine`, `PreemptiveTimeoutMs`), `Services/ConsoleWorker.cs` (`HandleExecuteAsync` busy-flip, `HandleGetCachedOutput`), `Services/ConsoleManager.cs` (`CollectCachedOutputsAsync` / `WaitForCompletionAsync` consumers), `Tools/ShellTools.cs::AppendCachedOutputs` (universal wrapper) |
