...
public class CommandTrackerTests
{
    public static void Run()
    {
        ...
        // UserCmdsSinceLastAi provenance counter — used by the proxy's drift
        // detector (ConsoleManager.PlanExecutionAsync) to decide whether the
        // live cwd still reflects AI's intended state or has been touched by
        // the human. Replaces the old "compare LastAiCwd vs live cwd" heuristic,
        // which false-positived when internal state lag (standby rotation, race
        // between RecordShellCwd and the next get_status) made AI's own cd
        // look like a user-initiated drift.

        // Provenance counter: fresh tracker starts at 0 and stays 0 across
        // pure AI traffic (RegisterCommand cycles, never a user command).
        {
            var t = new CommandTracker();
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: fresh tracker counter is 0");

            var (getLast, _) = CaptureSnapshot(t);
            _ = t.RegisterCommand(Reg("Get-Location"));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandExecuted));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.Cwd, cwd: "/tmp"));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            ...
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: counter stays 0 across pure AI command cycle");
        }

        // Provenance counter: user-typed pwsh command (B → C → D → A) bumps counter by 1.
        {
            var t = new CommandTracker();
            PrimeShellReady(t);
            ...
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 1, "provenance: pwsh user command increments counter to 1");

            t.HandleEvent(Evt(OscParser.OscEventType.CommandInputStart));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandExecuted));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 2, "provenance: a second pwsh user command increments counter to 2");
        }

        // Provenance counter: bash/zsh user-command pattern (C → D → A, no B) still bumps counter.
        {
            var t = new CommandTracker();
            PrimeShellReady(t);
            ...
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 1, "provenance: bash user command (C→D→A, no B) increments counter");
        }

        // Provenance counter: RegisterCommand resets counter back to 0.
        {
            var t = new CommandTracker();
            PrimeShellReady(t);
            ...
            t.HandleEvent(Evt(OscParser.OscEventType.CommandInputStart));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 1, "provenance: user cmd baseline = 1 before AI registers");

            var (getLast, _) = CaptureSnapshot(t);
            _ = t.RegisterCommand(Reg("Get-Date"));
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: RegisterCommand resets counter to 0");
            ...
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: still 0 after AI command cycle completes");
        }

        // Provenance counter: a bare OSC A (no preceding B/C) does NOT inflate
        // the count. Prevents shell-prompt repaints or spurious PromptStart
        // events from being misattributed as user commands.
        {
            var t = new CommandTracker();
            PrimeShellReady(t);
            ...
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: bare OSC A repeats do not increment counter");
        }

        // Provenance counter: OSC B fired during shell startup (before the
        // first OSC A) must NOT count as a user command. The integration
        // script emits an initial OSC B to mark the console as "ready"; that
        // marker predates any real user interaction.
        {
            var t = new CommandTracker();
            t.HandleEvent(Evt(OscParser.OscEventType.CommandInputStart));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 0, "provenance: startup OSC B → A does not increment counter");
        }

        // Provenance counter: interleaved user cycles between two AI commands —
        // counter accumulates user cycles and resets only on AI RegisterCommand.
        // Reproduces the scenario in the original bug report in spirit: AI runs
        // a cd, the human then runs their own command, and the next AI call's
        // get_status must see counter > 0 so the proxy treats the source as
        // drifted (user-owned) rather than trusting the live cwd.
        {
            var t = new CommandTracker();
            var (getLast, reset) = CaptureSnapshot(t);

            // First AI command: cd C:\MyProj\ripple
            _ = t.RegisterCommand(Reg("cd C:\\MyProj\\ripple"));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandExecuted));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.Cwd, cwd: "C:\\MyProj\\ripple"));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            var snap1 = getLast();
            if (snap1 != null) t.ReleaseAiCommand(snap1.Generation);
            Assert(t.UserCmdsSinceLastAi == 0,
                "provenance: after AI cd, counter stays 0 — AI's own cd is NOT misattributed to the user");

            // Now a human types a command in the same console.
            reset();
            t.HandleEvent(Evt(OscParser.OscEventType.CommandInputStart));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandExecuted));
            t.HandleEvent(Evt(OscParser.OscEventType.CommandFinished, exit: 0));
            t.HandleEvent(Evt(OscParser.OscEventType.PromptStart));
            Assert(t.UserCmdsSinceLastAi == 1,
                "provenance: a subsequent human command does increment counter — this is the signal the proxy uses to detect drift");
