            ...
            if (!_isAiCommand)
            {
                ...
                switch (evt.Type)
                {
                    ...
                    case OscParser.OscEventType.PromptStart:
                        // Closing a user-busy cycle: clear the busy flag so
                        // the next get_status reports idle. Drift detection
                        // is now done on the proxy side via direct cwd
                        // comparison (live cwd vs LastAiCwd), so we no
                        // longer need a per-event provenance counter here.
